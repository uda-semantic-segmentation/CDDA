...
from mmseg.models.uda.depthMetric import DepthMetric, celoss
...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        try:
            self.cdda = True
            self.croped_images_path = cfg['croped_images_path']
            self.croped_labels_path = cfg['croped_labels_path']
            self.depthMetric = DepthMetric(cfg)
            self.nor = torchvision.transforms.Normalize(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375])
        except:
            print('======Run without CDDA...')
            self.cdda = False
    ...
    def forward_train(self,
        ...
        if self.cdda:
            ...
            for i in range(batch_size):
                dep2 = target_img_metas[i]['dep'].data[0]
                target_img_metas[i].pop('dep', None)
                s2_deps.append(dep2)
                img_metas[i].pop('dep', None)
        ...
        if not self.source_only:
            ...
            pseudo_label, pseudo_weight, ps_large_p = self.get_pseudo_label_and_weight(
                ema_logits)
            del ema_logits


            if self.cdda:
                # Reweight and relabel
                pseudo_weight, new_src_img_idx, relabel_flag, tgt_onehot_lbl, tgt_pseudo_weight = \
                    self.depthMetric.depth_math(pseudo_weight, dev, batch_size, pseudo_label.clone(), s2_deps, ps_large_p,
                                                self.local_iter)
                del s2_deps, ps_large_p
            torch.cuda.empty_cache()
            pseudo_weight = self.filter_valid_pseudo_region(
                pseudo_weight, valid_pseudo_mask)
            gt_pixel_weight = torch.ones((pseudo_weight.shape), device=dev)
            if self.cdda:
                # Target missing classes guidance training
                if relabel_flag:
                    torch.cuda.empty_cache()
                    target_losses = self.get_model().forward_train(
                        target_img, target_img_metas, pseudo_label, tgt_pseudo_weight, return_logits=True)

                    relabel_tgt_loss = celoss(target_losses['decode.seg_logit'], tgt_onehot_lbl,
                                            torch.unsqueeze(tgt_pseudo_weight, dim=1))
                    relabel_tgt_loss.backward()
                    target_losses.pop('decode.seg_logit', None)
                    target_losses['loss'] = relabel_tgt_loss
                    target_losses = add_prefix(target_losses, 'tgt')
                    target_losses, tgt_log_vars = self._parse_losses(target_losses)
                    log_vars.update(tgt_log_vars)
                    del target_losses, relabel_tgt_loss, tgt_onehot_lbl, tgt_pseudo_weight

                # Change mixed source image
                ori_img, ori_gt_semantic_seg = img.cpu(), gt_semantic_seg.cpu()
                img, gt_semantic_seg = self.get_new_src(new_src_img_idx, img, gt_semantic_seg, dev)
                img = img.to(dev)
                gt_semantic_seg = gt_semantic_seg.to(dev)
            ...
            del gt_pixel_weight, strong_parameters
            if self.cdda:del img, gt_semantic_seg
            ...
            mix_loss.backward()
            if self.cdda:
                img, gt_semantic_seg = ori_img.to(dev), ori_gt_semantic_seg.to(dev)
        ...
        if self.enable_masking and self.mask_mode.startswith('separate'):
            ...
            del pseudo_weight, valid_pseudo_mask, img_metas, target_img_metas
    ...
    def get_new_src(self, new_src_idx, ori_imgs, ori_segs, dev):
        imgs, annos = [], []
        for i, idx in enumerate(new_src_idx):
            if idx == -1:
                imgs.append(ori_imgs[i].to(dev))
                annos.append(ori_segs[i].to(dev))
                continue
            idx = int(idx)
            # print(idx)
            img = cv2.imread(os.path.join(self.croped_images_path, f'{idx}.png'))
            anno = cv2.imread(os.path.join(self.croped_labels_path, f'{idx}.png')).transpose(
                2, 0, 1)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB).transpose(2, 0, 1)
            # img = mmcv.transforms.Normalize(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375],to_rgb=True)
            img = torch.tensor(img, dtype=torch.float)
            anno = torch.tensor(anno[0], dtype=torch.int64)
            anno = torch.unsqueeze(anno, dim=0)
            img = self.nor(img)
            imgs.append(img.to(dev))
            annos.append(anno.to(dev))
