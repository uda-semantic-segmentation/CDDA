...
class UDADataset(object):
    ...
    def synchronized_crop(self, s1, s2):
        ...
        for i, s in enumerate([s1, s2]):
            for key in ['img', 'gt_semantic_seg', 'valid_pseudo_mask', 'dep_info']:
                if key not in s:
                    continue
                s[key] = DC(
    ...
    def get_rare_class_sample(self):
        ...
        s1_dep = s1['dep_info']
        s2_dep = s2['dep_info']
        s1.pop('dep_info', None)
        s2.pop('dep_info', None)

        s2.pop('gt_semantic_seg',None)

        s1_tmp = s1['img_metas'].data
        s1_tmp['dep'] = s1_dep
        s2_tmp = s2['img_metas'].data
        s2_tmp['dep'] = s2_dep
